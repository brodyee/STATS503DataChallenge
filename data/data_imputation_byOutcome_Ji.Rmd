...
trainData = overallData[id.train, ]
testData = overallData[-id.train, ]
```

### filling missing values for train set
```{r message = FALSE, echo = FALSE, warning = FALSE}
trainData_admission_0 = subset(trainData, outcome == "recovered")
trainData_admission_1 = subset(trainData, outcome == "notRecovered")
...
for (i in 6:dim(trainData)[2]){
  ...
  if (grepl("mean", colnames(trainData)[i])){
      trainData_admission_0[is.na(trainData_admission_0[,i]),c(i,i+1,i+2)] = median(trainData_admission_0[!is.na(trainData_admission_0[,i]),i])
      trainData_admission_1[is.na(trainData_admission_1[,i]),c(i,i+1,i+2)] = median(trainData_admission_1[!is.na(trainData_admission_1[,i]),i])

  } else if (grepl("range", colnames(trainData)[i])| grepl("numTest", colnames(trainData)[i]) | grepl("changeStoF", colnames(trainData)[i])) {
      trainData_admission_0[is.na(trainData_admission_0[,i]),i] =0
      trainData_admission_1[is.na(trainData_admission_1[,i]),i] =0
  }

}

trainData <- full_join(trainData_admission_0, trainData_admission_1)
```


### filling missing values for test set
```{r message = FALSE, echo = FALSE, warning = FALSE}
testData_admission_0 = subset(testData, outcome == "recovered")
testData_admission_1 = subset(testData, outcome == "notRecovered")
...
for (i in 6:dim(testData)[2]){
  ...
  if (grepl("mean", colnames(testData)[i])){
      testData_admission_0[is.na(testData_admission_0[,i]),c(i,i+1,i+2)] = median(testData_admission_0[!is.na(testData_admission_0[,i]),i])
      testData_admission_1[is.na(testData_admission_1[,i]),c(i,i+1,i+2)] = median(testData_admission_1[!is.na(testData_admission_1[,i]),i])

  } else if (grepl("range", colnames(trainData)[i])| grepl("numTest", colnames(trainData)[i]) | grepl("changeStoF", colnames(trainData)[i])) {
      testData_admission_0[is.na(testData_admission_0[,i]),i] =0
      testData_admission_1[is.na(testData_admission_1[,i]),i] =0
  }

}

testData <- full_join(testData_admission_0, testData_admission_1)
```
