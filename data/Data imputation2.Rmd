...
train <- read.csv("Training Set for trainData.csv",header = T)
train.id = train$RecordID
test = data[-train.id,]
```
...
```{r}
resp1 <- test %>% filter(outcome == 1)
resp0 <- test %>% filter(outcome == 0)
```
...
m04 = resp0[,c(1:4,53:58)] %>% filter(AdmissionType==4, DiasABP_range <=40) %>% summarise(mean = mean(DiasABP_mean, na.rm=T))
...
n1 <- dim(resp1)[1]
n0 <- dim(resp0)[1]
m01 = resp0[,c(1:4,59:64)] %>% filter(AdmissionType == 1, FiO2_range <= 0.5) %>% summarise(mean = mean(FiO2_mean, na.rm = T))
...
m04 = resp0[,c(1:4,107:112)] %>% filter(AdmissionType == 4, MAP_range <=40) %>% summarise(mean = mean(MAP_mean, na.rm=T))
m0 = rbind(m0, c(2, as.numeric(m02[1])))
m0 = rbind(m0, c(4, as.numeric(m04[1])))
m12 = resp1[,c(1:4,107:112)] %>% filter(AdmissionType == 2, MAP_range <= 40) %>% summarise(mean = mean(MAP_mean, na.rm=T))
m1 = rbind(m1, c(2, as.numeric(m12[1])))
m14 = resp1[,c(1:4,107:112)] %>% filter(AdmissionType == 4, MAP_range <= 40) %>% summarise(mean = mean(MAP_mean, na.rm=T))
m1 = rbind(m1, c(4, as.numeric(m14[1])))
...
m04 = resp0[,c(1:4,185:190)] %>% filter(AdmissionType == 4, SysABP_range <=60) %>% summarise(mean = mean(SysABP_mean, na.rm=T))
m0 = rbind(m0, c(2, as.numeric(m02[1])))
m0 = rbind(m0, c(4, as.numeric(m04[1])))
m12 = resp1[,c(1:4,185:190)] %>% filter(AdmissionType == 2, SysABP_range <=60) %>% summarise(mean = mean(SysABP_mean, na.rm=T))
m1 = rbind(m1, c(2, as.numeric(m12[1])))
...
```{r}
Urinestable1 <- resp1[,c(1:4,209:214)] %>% filter(Urine_range <= 400)
Urinestable0 <- resp0[,c(1:4,209:214)] %>% filter(Urine_range <= 400)
...
```{r}
testdata <- rbind(resp1, resp0)
write.csv(testdata, "~/Desktop/DataChallenge/Test Set for trainData.csv", row.names = F)
```
